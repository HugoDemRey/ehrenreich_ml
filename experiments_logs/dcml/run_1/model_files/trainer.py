...
def train_one_epoch(model, temporal_contr_model, optimizer_model, optimizer_temp, train_loader, device, config, training_mode: str):
    ...
    for batch_idx, (data, labels, aug1, aug2) in enumerate(train_loader):
        ...
        if training_mode == 'self_supervised':
            ...
            # Keep original 3D features for temporal contrastive model
            feat1_3d = F.normalize(feat1, dim=1)
            feat2_3d = F.normalize(feat2, dim=1)

            temp_loss1, _ = temporal_contr_model(feat1_3d, feat2_3d)
            temp_loss2, _ = temporal_contr_model(feat2_3d, feat1_3d)

            # Flatten features for NTXentLoss (which expects 2D)
            feat1_flat = feat1.reshape(feat1.shape[0], -1) if len(feat1.shape) == 3 else feat1
            feat2_flat = feat2.reshape(feat2.shape[0], -1) if len(feat2.shape) == 3 else feat2
            ...
            loss = (temp_loss1 + temp_loss2) * 1.0 + nt_xent(feat1_flat, feat2_flat) * 0.7
