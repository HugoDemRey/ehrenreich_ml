...
class TC(nn.Module):
    ...
    def forward(self, features_aug1, features_aug2):
        ...
        # Ensure c_t is 2D for projection head (batch_size, hidden_dim)
        if len(c_t.shape) == 3:  # (batch, seq_len, hidden_dim)
            c_t = c_t.mean(dim=1)  # Average pool over sequence length
        elif len(c_t.shape) > 3:
            c_t = c_t.reshape(c_t.shape[0], -1)  # Flatten to 2D
