...
def compute_chromagram_from_filename(fn_wav, Fs=22050, N=4096, H=2048, gamma=None, version='STFT', norm='2'):
    ...
    if norm is not None:
        X = src.libfmp.c3.normalize_feature_sequence(X, norm=norm)
    Fs_X = Fs / H
...
def plot_chromagram_annotation(ax, X, Fs_X, ann, color_ann, x_dur, cmap='gray_r', title=''):
    ...
    """
    src.libfmp.b.plot_chromagram(X, Fs=Fs_X, ax=ax,
                             chroma_yticks=[0, 4, 7, 11], clim=[0, 1], cmap=cmap,
                             title=title, ylabel='Chroma', colorbar=True)
    src.libfmp.b.plot_segments_overlay(ann, ax=ax[0], time_max=x_dur,
...
def chord_recognition_template(X, norm_sim='1', nonchord=False):
    ...
    X_norm = src.libfmp.c3.normalize_feature_sequence(X, norm='2')
    chord_templates_norm = src.libfmp.c3.normalize_feature_sequence(chord_templates, norm='2')
    chord_sim = np.matmul(chord_templates_norm.T, X_norm)
    if norm_sim is not None:
        chord_sim = src.libfmp.c3.normalize_feature_sequence(chord_sim, norm=norm_sim)
...
def convert_chord_ann_matrix(fn_ann, chord_labels, Fs=1, N=None, last=False):
    ...
    """
    ann_seg_sec, _ = src.libfmp.c4.read_structure_annotation(fn_ann)
    ann_seg_sec = convert_chord_label(ann_seg_sec)
    ann_seg_ind, _ = src.libfmp.c4.read_structure_annotation(fn_ann, Fs=Fs, index=True)
    ann_seg_ind = convert_chord_label(ann_seg_ind)

    ann_frame = src.libfmp.c4.convert_ann_to_seq_label(ann_seg_ind)
    if N is None:
        N = len(ann_frame)
    if N < len(ann_frame):
