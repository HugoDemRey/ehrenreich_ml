...
from src.libfmp.c5 import get_chord_labels
...
def chord_recognition_all(X, ann_matrix, p=0.15, filt_len=None, filt_type='mean'):
    ...
    if filt_len is not None:
        if filt_type == 'mean':
            X, Fs_X = src.libfmp.c3.smooth_downsample_feature_sequence(X, Fs=1, filt_len=filt_len, down_sampling=1)
        if filt_type == 'median':
            X, Fs_X = src.libfmp.c3.median_downsample_feature_sequence(X, Fs=1, filt_len=filt_len, down_sampling=1)
    # Template-based chord recogntion
    chord_sim, chord_Tem = src.libfmp.c5.chord_recognition_template(X, norm_sim='1')
    result_Tem = src.libfmp.c5.compute_eval_measures(ann_matrix, chord_Tem)
    # HMM-based chord recogntion
    A = src.libfmp.c5.uniform_transition_matrix(p=p)
    C = 1 / 24 * np.ones((1, 24))
    B_O = chord_sim
    chord_HMM, _, _, _ = src.libfmp.c5.viterbi_log_likelihood(A, C, B_O)
    result_HMM = src.libfmp.c5.compute_eval_measures(ann_matrix, chord_HMM)
