...
def compute_cens_from_chromagram(C, Fs=1, ell=41, d=10, quant=True):
    ...
    """
    C_norm = src.libfmp.c3.normalize_feature_sequence(C, norm='1')
    C_Q = quantize_matrix(C_norm) if quant else C_norm

    C_smooth, Fs_CENS = src.libfmp.c3.smooth_downsample_feature_sequence(C_Q, Fs, filt_len=ell,
                                                                     down_sampling=d, w_type='hann')
    C_CENS = src.libfmp.c3.normalize_feature_sequence(C_smooth, norm='2')
...
def compute_cens_from_file(fn_wav, Fs=22050, N=4410, H=2205, ell=21, d=5):
    ...
    X_CENS, Fs_CENS = src.libfmp.c7.compute_cens_from_chromagram(X_chroma, Fs=Fs/H, ell=ell, d=d)
    L = X_CENS.shape[1]
...
def compute_matching_function_dtw(X, Y, stepsize=2):
    ...
    Returns:
        ...
        D (np.ndarray): Accumulated cost matrix
    """
    C = src.libfmp.c7.cost_matrix_dot(X, Y)
    if stepsize == 1:
        D = src.libfmp.c7.compute_accumulated_cost_matrix_subsequence_dtw(C)
    if stepsize == 2:
        D = src.libfmp.c7.compute_accumulated_cost_matrix_subsequence_dtw_21(C)
    N, M = C.shape
    Delta = D[-1, :] / N
    return Delta, C, D
...
def matches_dtw(pos, D, stepsize=2):
    ...
    for k in range(len(pos)):
        ...
        matches[k, 1] = t
        if stepsize == 1:
            P = src.libfmp.c7.compute_optimal_warping_path_subsequence_dtw(D, m=t)
        if stepsize == 2:
            P = src.libfmp.c7.compute_optimal_warping_path_subsequence_dtw_21(D, m=t)
        s = P[0, 1]
        matches[k, 0] = s
    return matches
