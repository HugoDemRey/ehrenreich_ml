...
def compute_freq_distribution(x, Fs, N=16384, gamma=100.0, local=True, filt=True, filt_len=101):
    ...
    if local:
        ...
        Y, T_coef, F_coef = src.libfmp.c2.stft_convention_fmp(x=x, Fs=Fs, N=N, H=N//2, mag=True, gamma=gamma)
        ...
        Y = Y[:N//2+1]
        Y = np.log(1 + gamma * Y)
        # Y = src.libfmp.c3.log_compression(Y, gamma=100)
    ...
    F_coef_log, F_coef_cents = src.libfmp.c2.compute_f_coef_log(R=1, F_min=F_min, F_max=F_max)
