...
def plot_feature_ssm(X, Fs_X, S, Fs_S, ann, duration, color_ann=None,
    ...
    Args:
        ...
        ann: Annotaions
        duration: Duration
        color_ann: Color annotations (see :func:`src.libfmp.b.b_plot.plot_segments`) (Default value = None)
    ...
    fig, ax = plt.subplots(3, 3, gridspec_kw={'width_ratios': [0.1, 1, 0.05],
                           ...
                           figsize=figsize)
    src.libfmp.b.plot_matrix(X, Fs=Fs_X, ax=[ax[0, 1], ax[0, 2]], clim=clim_X,
                         xlabel='', ylabel='', title=title)
    ax[0, 0].axis('off')
    src.libfmp.b.plot_matrix(S, Fs=Fs_S, ax=[ax[1, 1], ax[1, 2]], cmap=cmap, clim=clim,
                         title='', xlabel='', ylabel='', colorbar=True)
    ax[1, 1].set_xticks([])
    ax[1, 1].set_yticks([])
    src.libfmp.b.plot_segments(ann, ax=ax[2, 1], time_axis=time, fontsize=fontsize,
                           colors=color_ann,
                           time_label=label, time_max=duration*Fs_X)
    ax[2, 2].axis('off')
    ax[2, 0].axis('off')
    src.libfmp.b.plot_segments(ann, ax=ax[1, 0], time_axis=time, fontsize=fontsize,
...
def subplot_matrix_colorbar(S, fig, ax, title='', Fs=1,
    ...
    """
    if cmap is None:
        cmap = src.libfmp.b.compressed_gray_cmap(alpha=-100)
...
def shift_cyc_matrix(X, shift=0):
    ...
    X_cyc = np.zeros((K, N), dtype=X.dtype)  # Preserve input dtype
...
def compute_sm_ti(X, Y, L=1, tempo_rel_set=np.asarray([1]), shift_set=np.asarray([0]), direction=2):
    ...
    for shift in shift_set:
        ...
        S_cyc = src.libfmp.c4.compute_sm_dot(X, Y_cyc)

        if direction == 0:
            S_cyc = src.libfmp.c4.filter_diag_mult_sm(S_cyc, L, tempo_rel_set, direction=0)
        if direction == 1:
            S_cyc = src.libfmp.c4.filter_diag_mult_sm(S_cyc, L, tempo_rel_set, direction=1)
        if direction == 2:
            S_forward = src.libfmp.c4.filter_diag_mult_sm(S_cyc, L, tempo_rel_set=tempo_rel_set, direction=0)
            S_backward = src.libfmp.c4.filter_diag_mult_sm(S_cyc, L, tempo_rel_set=tempo_rel_set, direction=1)
...
def compute_sm_from_filename(fn_wav, L=21, H=5, L_smooth=16, tempo_rel_set=np.array([1]),
    ...
    X, Fs_feature = src.libfmp.c3.smooth_downsample_feature_sequence(C, Fs_C, filt_len=L, down_sampling=H)
    X = src.libfmp.c3.normalize_feature_sequence(X, norm='2', threshold=0.001)

    # Compute SSM
    S, I = src.libfmp.c4.compute_sm_ti(X, X, L=L_smooth, tempo_rel_set=tempo_rel_set, shift_set=shift_set, direction=2)
    S_thresh = src.libfmp.c4.threshold_matrix(S, thresh=thresh, strategy=strategy,
