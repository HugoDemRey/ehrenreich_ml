...
def colormap_penalty(penalty=-2, cmap=src.libfmp.b.compressed_gray_cmap(alpha=5)):
    ...
    Args:
        ...
        cmap (mpl.colors.Colormap): Original colormap (Default value = src.libfmp.b.compressed_gray_cmap(alpha=5))
...
def plot_ssm_ann(S, ann, Fs=1, cmap='gray_r', color_ann=[], ann_x=True, ann_y=True,
    ...
    Args:
        ...
        color_ann: color scheme used for annotations (see :func:`src.libfmp.b.b_plot.plot_segments`)
            (Default value = [])
    ...
    fig_im, ax_im, im = src.libfmp.b.plot_matrix(S, Fs=Fs, Fs_F=Fs,
    ...
    if ann_y:
        src.libfmp.b.plot_segments_overlay(ann, ax=ax_im[0], direction='vertical',
                                       time_max=S.shape[0]/Fs, print_labels=False,
                                       colors=color_ann, alpha=0.05)
    if ann_x:
        src.libfmp.b.plot_segments(ann, ax=ax[1, 0], time_max=S.shape[0]/Fs, colors=color_ann,
...
def compute_fitness_scape_plot(S):
    ...
    for length_minus_one in range(N):
        for start in range(N-length_minus_one):
            ...
            D, score = src.libfmp.c4.compute_accumulated_score_matrix(S_seg)
            path_family = src.libfmp.c4.compute_optimal_path_family(D)
            fitness, score, score_n, coverage, coverage_n, path_family_length = src.libfmp.c4.compute_fitness(
                path_family, score, N)
...
def plot_seg_in_sp(ax, seg, S=None, Fs=1):
    ...
    if S is not None:
        ...
        D, score = src.libfmp.c4.compute_accumulated_score_matrix(S_seg)
        path_family = src.libfmp.c4.compute_optimal_path_family(D)
        segment_family, coverage = src.libfmp.c4.compute_induced_segment_family_coverage(path_family)
...
def plot_sp_ssm(SP, seg, S, ann, color_ann=[], title='', figsize=(5, 4)):
    ...
    """
    float_box = src.libfmp.b.FloatingBox()
    ...
    penalty = np.min(S)
    cmap_penalty = src.libfmp.c4.colormap_penalty(penalty=penalty)
    fig, ax, im = src.libfmp.c4.plot_ssm_ann_optimal_path_family(
...
def check_segment(seg, S):
    ...
    fitness, score, score_n, coverage, coverage_n, path_family_length = src.libfmp.c4.compute_fitness(
                path_family, score, N)
    segment_family, coverage2 = src.libfmp.c4.compute_induced_segment_family_coverage(path_family)
